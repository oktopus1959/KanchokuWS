...
$BINDIR/colorcat.sh 'Determiner.*(ENTER|LEAVE)|COMBO.*(PASSED|FOUND|NOT|FAILED)|(result|hotList|comboList|unprocList|challengeList)=[0-9:RS/]+|XXXX\
|TIMER. (STARTED|ELAPSED)|TRY NEXT:.*|(NO )?RULE[ (].*|bTemporaryComboDisabled=True\
...
| CurrentLine:[0-9]+:.*|Analyze for .* mode|extraInfo=1959|isCombinationTiming(.* RESULT.=\w+)?|TIMER-. ELAPSED:|SendInputHandler.\w+\
