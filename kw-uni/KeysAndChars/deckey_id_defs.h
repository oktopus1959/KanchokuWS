...
// このファイルは tools/make_deckey_id_def.sh により KanchokuWS/DecoderKeys.cs から自動的に作成されました (2022/04/23 15:54:59)
#pragma once
...
#define SHIFT_M_DECKEY_START (SHIFT_B_DECKEY_END)

// 同時打鍵用SHIFT_M修飾DECKEYの終わり(の次)
#define SHIFT_M_DECKEY_END (SHIFT_M_DECKEY_START + SHIFT_DECKEY_NUM)

// シフト面の数(通常面を含む)
#define ALL_SHIFT_PLANE_NUM (5)

// SHIFTキーの総数
#define TOTAL_SHIFT_DECKEY_NUM (SHIFT_DECKEY_NUM * (ALL_SHIFT_PLANE_NUM - 1))

// ストロークキーの数
#define STROKE_DECKEY_NUM (NORMAL_DECKEY_NUM + TOTAL_SHIFT_DECKEY_NUM)

// ストロークキーの終わり(の次)
#define STROKE_DECKEY_END (STROKE_DECKEY_NUM)
