...
#### 疑似同時打鍵サポート
これを有効にすると SandS で Space キー押下がシフト状態になった時に疑似的な空白コードをデコーダに送信します。

デコーダは、第2打鍵(以降の)待ちのときに疑似空白を受信すると、それを通常の空白コードとして扱います。
第1打鍵待ちのときはそれを無視します。
たとえば、「A, Space(押したまま), B」という順序で打鍵した時に、Space が Shift 状態
に変化する時点で疑似空白コードが発生し、 A→空白という打鍵列に
相当する文字 (Shift+A と同じ文字)が出力されることになります。

これにより、「Shift+A」と「A→空白」の両方に同じ文字を割り当てておけば、
「Space, A, B」という順序で押した場合でも「A, Space, B」
という順序で押した場合でも同じ文字が出力されるようになります。

なお、矢印束記法(-*>-nn>) を用いると、Shift面に割り当てる文字集合と、
第2打鍵が空白となる第1打鍵文字集合とを簡単に定義できるようになります。
