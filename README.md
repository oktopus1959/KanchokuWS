...
|ファイル名|コード名|特徴|
|-|-|
|t.tbl|T-Code|2ストローク、カタカナあり、覚えにくい|
...
有効にしたい場合は、設定ダイアログの
「履歴・交ぜ書き・他」-「その他変換・機能」-「デコーダON時に SandS を有効にする」
にチェックを入れてください。


## 既知のバグ
- 交ぜ書き変換で、先頭文節が変換形と同じ読みであり、次文節が実際に変換したい読みである場合に、
変換の第1候補に次文節の変換結果が出てこない
  - 例：「とてもひさんな」を交ぜ書き変換すると、第1候補が読みと同形になる。「とても悲惨な」が期待する結果。
