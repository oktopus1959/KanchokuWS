...
- [その他の機能](#その他の機能)
    ...
    - [Spaceキーによる Shift機能をサポート (SandS)](#Spaceキーによる-Shift機能をサポート-SandS)
        - [SandS有効時にSpace単打で空白を入力しない](#SandS有効時にSpace単打で空白を入力しない)
...
この文字列に `!{keyName}`というような文字列を記述すると、`keyName` に相当する機能キーを出力できます。

例：テーブルファイルに、後述の矢印記法を用いて
```
-7>-8>"（）!{left}"
```
という記述をすると、これは(Qwerty配列で) `89` と打鍵することで `（）` を出力し、
...
#### 複数のシフト面
従来シフト面の他に、シフトA面とシフトB面が追加されました。面としては、
...
`#yomiConvert with "filename"` と記述すると、上記の読み変換定義を、
`filename` の内容に記述されたマッピングに従った変換で上書きします。 
たとえば、`書 粛` というマッピングがあると、読みファイルによる定義 `書 しょ` が上書きされます。
上書き定義の具体例については、同梱の `user-yomi.txt` を参照してください。
...
また、ストロークが割り当てられていないが良く使用する文字を、
連想される文字の変換文字として裏定義しておくと便利かと思います。
（例：「田」の裏定義で「畑」を設定しておくなど）
...
副テーブルファイルは、下図のように設定ダイアログの「基本設定」>「ファイル」>「テーブルファイル2」
で設定します。
...
|数文字入力履歴検索|1/?|1 は1文字の履歴候補の表示と選択<br/>? は1～3文字の履歴候補の表示と選択<br/>(ともに容易打鍵文字のみの候補を除く)|
|交ぜ書き変換|M|後置式の交ぜ書き変換|
|部首合成|B|後置式の部首合成|
...
設定ダイアログの「機能キー割当」タブで、打鍵列を設定するとよいでしょう。
...
拡張シフトA面(A00～A49)、拡張シフトB面(B00～B49)および特殊機能キー(X1～X7)があります。

たとえば、106配列において、「a」は 20、「Shift+Space」は S40、「拡張シフトA+@」は A44
と記述します。
また、特殊機能キーは名前でも記述できます。
...
また、最初から1文字だけの履歴候補を表示する機能、1～3文字だけの履歴を表示する機能もあります。
下図の設定ダイアログを参照してください。
...
拡張修飾キーで LeftShiftBlocker や RightShiftBlocker を呼び出すことでも交ぜ書きブロッカーを移動させることができます。
...
拡張修飾キーで LeftShiftMazeStartPos や RightShiftMazeStartPos を呼び出すことでも交ぜ書き開始位置を移動させることができます。

### 空白文字を変換開始位置とする
半角空白文字の後に読みを入力することで、変換開始位置を制御することもできます。
上記の「ひどい目にあう」の例だと「ひどい目に<空白>あう」(実際には「ひどい目に あう」)と入力してから
交ぜ書きを実行すると、一発で「ひどい目に遭う」と変換することができます。

この機能を有効にするのは、設定ダイアログの「履歴・交ぜ書き」-「交ぜ書き変換」-
「空白文字を変換開始位置とする」にチェックを入れてください。
...
この機能を有効にするには、設定ダイアログの「その他設定」>「自動部首合成」>「自動部首変換を有効にする」
をONにしてください。
...
### 合成元の2文字に戻す
自動部首合成の実行直後に Escキーを押すか、または「連想直接入力」を呼び出すと、
合成された文字を削除して元の2文字に戻します。
たとえば、「メリ」で「刈」となるような登録がされている状態で「メリット」と入力したい場合は、
「メリ」⇒「刈」となった時点で Esc キーを押すか「連想直接入力」を呼び出して「メリ」に戻し、
続けて「ット」と入力します。

さらに続けて Esc キーを押すか「連想直接入力」を呼び出すと、合成元の2文字の組み合わせを無効化登録して、
自動部首合成の対象から外します。

### 辞書登録
自動部首合成辞書登録により、部首合成辞書にない組み合わせを登録したり、
登録済みの組み合わせを無効化したりできます。
設定ダイアログの「辞書登録」-「自動部首合成登録」で登録してください。
...
なお「余白情報表示」ボタンをクリックすると下図のような現在の余白情報が表示されます。
これを参考にして余白を設定してください。
...
a) 設定ダイアログの「その他設定」-「打鍵ガイド」-「大文字ローマ字読みによる打鍵ガイドを常時有効」をONにする
...
SandS を通常シフト面以外の拡張シフト面に割り当てたい場合は、次項「拡張修飾キー」の説明を参照してください。

#### SandS有効時にSpace単打で空白を入力しない
これにチェックを入れると、Spaceキー単打では空白文字が入力されなくなります。
つまり、ほぼ通常のShiftキーと同等の使い方ができるようになります。
...
これは上記の「Space単打で空白を入力しない」の設定に関わらず有効となります。

#### 疑似同時打鍵サポート
これを有効にすると SandS で Space キー押下がシフト状態になった時に後置シフト(疑似)コードをデコーダに送信します。

デコーダは、第2打鍵待ちのときに後置シフトコードを受信すると、いったん第1打鍵をキャンセルした上で
それをシフト打鍵に置換します。 第1打鍵待ちのときは後置シフトコードを無視します。

たとえば、「A, Space(押したまま), B」という順序で打鍵した時に、
Space が Shift 状態に変化する時点で後置シフトコードが発生し、
第1打鍵の A は Shift+A に置換されます。
これにより、「Space, A, B」という順序で押した場合でも「A, Space, B」
という順序で押した場合でも同じ文字が出力されるようになります。


### 機能キーを修飾キーとして使う(拡張修飾キー)
Space(SandS有効時)、CapsLock, 英数, 無変換, 変換, 右シフトの6つの機能キーを修飾キーとして使用し、
通常シフト面、拡張シフト面A、同B のいずれかに割り当てることができます。

また、これらのキーを押しながら漢直ストロークキーを押した時に、
Esc, Tab, BackSpace, Enter, Insert, Delete, Home, End, PageUp, PageDown, UpArrow, DownArrow, LeftArrow, RightArrow
キーのいずれかのキーに変換することもできます。使用できるキー名については、
[キーボードファイルによる機能キーの仮想キーコード定義](#キーボードファイルによる機能キーの仮想キーコード定義)
を参照してください。

また、上記のキーおよび "A"～"Z" のキー名の頭に "^" を付加することで、 Ctrl修飾されたキーを表すことができます。
たとえば、"^Left" は「Ctrl＋左矢印」に、"^S" は「Ctrl＋S」になります。

また、下記の特殊機能の呼び出し(コマンド)も割り当てることができます。
...
設定ダイアログの「その他設定」>「ローマ字テーブルファイル出力」のところの「出力」ボタンをクリックします。
