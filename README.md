...
上記機能の呼び出し打鍵列は、テーブルファイルに「@{機能標識}」を記述する方法の他に、
設定ダイアログの「機能キー割当」タブでも設定することができます。

※初期状態の kanchoku.user.ini には上記の機能について、デフォルトの呼び出し打鍵列が設定されています。
これはテーブルファイルの機能呼び出し設定を上書きするので、利用者の想定と異なっている可能性があります。
...
タグ部分に -Y-ADD/-Y-SUB が含まれるのが Yアルゴ、T/U で始まるのが再帰足し算です。
