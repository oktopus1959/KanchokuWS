...
カレットからの相対的な表示位置を設定ダイアログの「詳細設定」>「仮想鍵盤・モード標識」
...
※ 仮想鍵盤をマウスでドラッグすることにより、任意の固定位置に表示させることもできます。

![Vkb Mode Marker Waittime](image/vkb-mode-marker-waittime.png)
...
0xeb (VK_OEM_PA1)になります。このようなケースにも対応できるようになります。
...
#### 容易打鍵文字からなる候補の劣後
設定ダイアログの「基本設定」-「ファイル」-「容易打鍵文字ファイル」で指定したファイルに記述した
容易打鍵文字だけを含むような候補は、候補列の後尾に回されます。
...
容易打鍵文字として、上中下段の30x30=900文字を一括で指定することもできます。
設定の詳細については、`easy_chars.txt` ファイルを参照してください。
