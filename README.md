...
|カタカナ入力|K/k|Kはカタカナ変換モード、k は一括置換|

### 打鍵列への割り当て
上記機能の呼び出しは、以下の方法で打鍵列に割り当てることができます。
- テーブルファイルに「@{機能標識}」（例：`@M`）を記述する
- 設定ダイアログの「機能キー割当」タブでも設定する

#### テーブルファイルに記述
これは従来からある方法となります。

なお、既存のテーブルファイルをUTF-8に変換してそのまま利用することもできますが、その場合、
...
#### 表示スケール
高精細ディスプレイなどを使用しており、表示スケールを 1.25 などに変更している場合は、
取得したカレット位置(物理座標)が実際のスクリーン上の位置とずれることがあります。

そのような場合は、「設定」-「詳細設定」-「仮想鍵盤・モード標識」-「ディスプレイ表示スケール」
の値を調整してください。

#### 相対位置の個別設定
上記の調整をしても、対象アプリによっては仮想鍵盤の表示位置がずれる場合があります。
とくに Word や Excel といった Office アプリは、カレットの上に仮想鍵盤が乗ってしまうケースが多いようです。

そのような場合は、対象アプリ（ウィンドウ）ごとに、仮想鍵盤の表示位置（カレットからの相対位置）
を設定してください。設定方法は下記のとおりです。
...
3. 上記のウィンドウクラス名をセクション名として新しいセクションを追加する
...
これで kanchoku.\*.init の変更が漢直窓Sに反映されます。
...
というような注意を念頭に置いた上で、 AutoHotKey によるキー変換機能に満足できない方、具体的には
...
- テーブルファイルの分割と矢印記法のサポート
  - a→α、b→βとか、TUT互換の記号テーブルとか
  - ファイル分割により異なるコード系間での流通が容易になる
- ミニバッファ編集
