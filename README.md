...
辞書の保存時には、既存の辞書ファイルが `back` ディレクトリに保存されます。
...
拡張シフトA面(A00～A49)、拡張シフトB面(B00～B49)、および拡張修飾キー(X1～X7)と右シフトの単打があります。
...
### 履歴の手動登録
ひらがなを含む語や、漢字・カタカナ混在語、短縮形、あるいはいったん削除した語は、手動で登録します。
登録方法には下記の2通りがあります。
...
設定ダイアログの「その他設定」>「自動部首合成・連想直接出力」>「連想直接出力の試行回数」で設定してください。
...
この文字列をコピーして仮想鍵盤のミニバッファにペーストするか、
この文字列を選択状態にしておいて CopySelectionAndSendToDictionary を呼び出すか、
あるいは「部首連想登録」のところで直接入力して「登録」ボタンを押します。
...
### 合成の次候補出力
部首合成の実行直後に上述の「連想直接出力」を呼び出すと、次の合成候補出力になります。
...
ただし、何か文字を入力した直後、矢印キーやマウスによるカーソル移動をして別の文字を入力しようとした時に
意図しない自動合成が発動することもありますので、留意ください。
...
### 合成元の2文字に戻す
自動部首合成の実行直後に Escキーを押すか、または「連想直接出力」を呼び出すと、
合成された文字を削除して元の2文字に戻します。
たとえば、「メリ」で「刈」となるような登録がされている状態で「メリット」と入力したい場合は、
「メリ」⇒「刈」となった時点で Esc キーを押すか「連想直接出力」を呼び出して「メリ」に戻し、
続けて「ット」と入力します。

さらに続けて Esc キーを押すか「連想直接出力」を呼び出すと、合成元の2文字の組み合わせを無効化登録して、
自動部首合成の対象から外します。
...
そのような場合は、「縦列鍵盤表高さ調整率」を変更してみてください。
