...
|引数|説明|
|--|--|
|preShift|前置シフトかつ連続シフトあり（前置連続シフト）|
|mutual|相互シフトかつ連続シフトあり（相互連続シフト; デフォルト）|
|oneshot|相互シフトかつ連続シフトなし（相互ワンショットシフト）|

**前置シフト**とは、シフトキーが前置されるものをいいます。上記の例でいえば、キーコード23(F)と
...
もし、上記の例で、`#combination preShift` ではなく、
`#combination mutual` (またはたんに `#combination`)だった場合は、
...
例として「mod-conversion.txt」というファイルを用意してあります。
「開く」ボタンをクリックすると、テキストエディタでそのファイルを開いて内容を確認することができます。

「設定」ボタンをクリックすると、下図のような「拡張修飾キー設定」ダイアログが開きます。
拡張修飾キーに対する割り当てを修正する場合は、こちらを使うほうが便利です。
...
もし、複数のキー割り当てを一括で変更する場合は、定義ファイルを直接修正したほうが簡単かもしれません。
修正完了後、「再読込」をクリックすると、設定ダイアログのほうにも修正結果が反映されます。

なお、定義ファイルに記述する場合の書式などについては `mod-conversion.sample.txt` 
というファイルに詳細を記述してあるので、こちらを参照してください。
...
SandS, CapsLock, 英数, 無変換, 変換, 右シフトの6つの機能キーに対して、
通常シフト面、拡張シフト面A、同B のいずれかに割り当てることができます。

上部の「拡張修飾キー」コンボボックスで対象のキーを選択し、
「漢直ON時シフト面」「漢直OFF時シフト面」コンボボックスで割り当てる拡張シフト面を選択します。
