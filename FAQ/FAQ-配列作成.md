...
`X` のところには、漢直キーコードの他に、書き換え対象となる前置文字を記述することもできます。

上図では文字「n」を直接指定していますが、下記のように漢直キーコードを使っても同じ結果が得られます。
...
まず共通の後置キーを `&X>` という形式の記法で記述します。矢印記法に似ていますが、
先頭部が `&` になっています。
...
「ローマ字配列」での `Q` 単打は「q」なのですが、その直後に `A` を打鍵すると、
...
本質的には下記で説明する「より高度な記述法」における「ウ段」の `lt: っ` などと同じです。
...
### より高度な記述法
書き換え対象が複数文字からなっていたり、同時打鍵で書き換えを行いたいような場合は、
書き換え対象文字列と書き換え後の文字列のペアを直接記述することもできます。

次図は、ローマ字配列で、`ltu` や `xtu` を「っ」に書き換える例です。

![Post Rewrite Roman U](image/post-rewrite-roman-u.png)

次図は、「M+カンマ」の同時打鍵で、「き」を「きゃ」などに書き換える例です。

![Post Rewrite Pair](image/post-rewrite-pair.png)

上図のように `{` の直後に文字列を記述すると書き換えパターンにマッチしない場合には、
その文字列が出力されます。上図の場合だと、直前の出力文字が「き」「に」「り」以外の場合は、
`!{Left}` すなわち左矢印キーが出力されます。
...
### 書き換え対象文字の出力を確定させたい
「。」などが前置書き換え対象文字である場合に、次のキー入力でそれを書き換えられないようにする方法です。
...
前置書き換え対象文字が入力されてからの経過時間によって自動的に確定させることもできます。

設定ダイアログ >「同時打鍵・IME」>「書き換えシステム」で
「前置書き換えを許容する遅延時間(ミリ秒)」に入力文字確定までの時間を設定してください。
