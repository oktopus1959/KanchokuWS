...
- [シフト打鍵用テーブル](#シフト打鍵用テーブル)
    ...
    - [漢直モードがOFFの時も「IMEオン」の同時打鍵列を有効にしたい](#漢直モードがOFFの時もIMEオンの同時打鍵列を有効にしたい)
...
### 漢直モードがOFFの時も「IMEオン」の同時打鍵列を有効にしたい
次のように、同時打鍵定義のところを `#enableAlways` と `#end enableAlways` で囲んでください。
下記は、漢直モードが OFF の時でも、`JH` の同時押しで IME ON, `FG` の同時押しで IME OFFにする設定です。
