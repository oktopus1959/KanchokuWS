...
- [矢印記法とは何ですか](#矢印記法とは何ですか)
    ...
    - [`Enter` や矢印キーのような機能キーを定義したい](#`Enter`-や矢印キーのような機能キーを定義したい)
    - [特殊機能の割り当て](#特殊機能の割り当て)
    - [仮想キーコード記述](#仮想キーコード記述)
...
- [`define` と `alias` (`defineKey`)](#define-と-alias-defineKey)
    - [`define`](#define)
    - [`alias` (`defineKey`)](#alias-defineKey)
...
### 任意キーによるホールドシフト
Tab キーや Enter キーなどにシフトキーと同様の機能を持たせ、拡張シフト面に割り当てる設定です。
下記のように、 `#HoldShift`  ～ `#end HoldShift` の間に定義を書きます。

`その他/tab-hold-shift.tbl`:
```
;; TAB によるホールドシフト
#HoldShift TAB E both show
{
   1  |  2  |  3  |   4   |  5   |  6  | "(" |  8  |  9  | ")" | "=" | "～"| "\\"|
      |{TAB}|{End}|   0   |{TAB} | "%" |  5  |  6  |  7  |  0  | "-" | "[" |
{Home}|     |{Del}|{Right}|{ESC} |{BS} |  1  |  2  |  3  |  4  | ":" | "]" |
      |     |     |       |{Left}|  0  |  0  | "," | "." | "/" | "_" |
}
#end HoldShift
```

`#HoldShift {KEY} {PLANE} {OPTION}...`  の書き方は以下のようになります。

- KEY: 配列キーコードまたはプレースホルダー名
- PLANE: 定義を割り当てる拡張シフト面の名前 (`A` ~ `F`)
- OPTION: `both` [`show`] 
    - `both`: デコーダがONの場合もOFFの場合も有効となる (指定しなければ ON の場合のみ有効)
    - `show` : `both` 設定時に、デコーダがOFFの場合でも、KEY を長押しした時に仮想鍵盤に下図のような打鍵ヘルプを表示する

![alt text](image/hold-shift.png)

KEY として利用できるプレースホルダー名については「[既定のプレースホルダー](#既定のプレースホルダー)」を参照してください。
...
英数モードと漢直モードの両方で有効にしたい場合は、`#enableAlways` ～ `#end enableAlways` で囲んでください。
...
これらは `$` なしでも使用することができます。また、大文字/小文字を区別しません。<br>
下記は `Enter` をプレースホルダー名として利用する例です。
```
-Enter>{ ... }
```
...
|caps|Caps Lock|53|
|alnum / eisu|英数|54|
|nfer / muhenkan|無変換|55|
|xfer / henkan|変換|56
|kana / hiragana|ひらがな|57|
|bs / back / backspace|Back Space|58|
|enter|Enter|59|
|ins / insert|Insert|60|
|del / delete|Delete|61|
|home|Home|62|
|end|End|63|
|pgup / pageup|Page Up|64|
|pgdn / pagedown|Page Down|65|
|up / uparrow|↑|66|
|down / downarrow|↓|67|
|left / leftarrow|←|68|
|right / rightarrow|→|69|
|pause|Pause|70|
|scrlock|Scroll Lock|71|
|imeon|IME On|72|
|imeoff|IME Off|73|
|lctrl|左Ctrl|74|
|rctrl|右Ctrl|75|
...
### 特殊機能の割り当て
交ぜ書き変換、部首合成、カタカナ変換といった特殊機能を割り当てることもできます。

下記は、論理コード `47` (Qewrtyの `\`) に「カタカナ変換」機能を割り当てる例です。
```
-47>"!{KatakanaConversion}"
```
...
### 簡略記法
テーブルのブロック記法とまぎれのない場合は `"!{keyName}"` を `{keyName}` のように書くことができます。
外側を囲むダブルクォートと前置される `!` が不要になります。

下記は、論理キー `46` (Qwertyの `]`) に部首合成を割り当てる例です。
```
-46>{BushComp}
```
...
## `define` と `alias` (`defineKey`)
### `define`
`define` は、テーブル定義のブロック外 (つまり `{ ... }` の外) の世界での別名定義です。
`#if` のオペランドや矢印記法でのプレースホルダーの別名を定義したりするのに用います。

以下は、`Enter` というプレースホルダーに対して `改行` という別名を定義して使用している例です。
プレースホルダーとして使用しているので、 `$` が必要になっている点に注意。
...
### `alias` (`defineKey`)
`alias`は、アプリに対して送出される機能キーの別名を定義するのに用います。

以下は、`U` に対して `BS` (Back Space) を設定している例です。`BS` に `後退` という別名を定義しています。
この例のように、`{ }` で囲んだ形で使用します。
...
なお、`defineKey` は `alias` の別名です。`alias` と全く同様に使用できます。
