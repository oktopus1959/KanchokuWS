...
- [使用するキーとその論理キーコード](#使用するキーとその論理キーコード)
- [単打用テーブル](#単打用テーブル)
...
テーブルファイルには、キーを表す「論理キーコード」とそれに対応する出力文字(列)を記述します。
...
背景色の付いた数字は、各キーに割り振られた「**論理キーコード**」を示しています。

![Keyboard Layout](image/keyboard-layout.png)

論理キーコードと出力文字(列)の対応は「矢印記法」
...
`{` と `}` で囲まれたところに、論理キーコード 0 ～ 48 のキーに対応する文字（あるいは文字列）を、
...
[矢印記法](#矢印記法とは何ですか)で前置シフトキーの論理キーコードを記述した後、単打の場合と同様にテーブル構造を記述します。
なお `;`
...
-{論理キーコード}>"出力文字列"
```
`{論理キーコード}`にはキーに割り振られた 0～99 の[論理キーコード](../KEYBOARD.md)を記述します。たとえば、
```
-10>"あ"
```
...
### 簡略記法
論理キーコードが 0～39 および 41, 44, 46 のものについては、`$a` や `$.` といった簡略記法が可能です。
...
-a,$.>ぬ
```
上記例で、`a` の後の `,` は、論理キーコードの並びを区切るカンマであることに注意。
...
それらのキーに対応する論理キーコードを[矢印記法](#矢印記法とは何ですか)のところに記述します。

![Nicola Nfer](image/nicola-nfer.png)

![Nicola Xfer](image/nicola-xfer.png)

「無変換」などの特殊キー(機能キー)の論理キーコードは 50～99 となりますが、`X0～X49` と書くこともできます。
