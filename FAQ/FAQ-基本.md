...
- [英数字数号配列の選択](#英数字数号配列の選択)
    - [独自の英数字配列を使いたい](#独自の英数字配列を使いたい)
- [漢直WSを一時的に停止したい](#漢直WSを一時的に停止したい)
    - [Pauseキーで一時停止・再開する](#Pauseキーで一時停止・再開する)
- [仮想鍵盤](#仮想鍵盤)
...
英語キーボードを選択する場合は、OSの設定で「英語キーボード(101/102キー)」を選択しておいてください
...
- 先頭行の `#NAME: ～` は、ファイル選択時のドロップダウンに表示される名前を設定します
- `MODE=US` で英語キーボードであることを宣言しています。日本語キーボードの場合は不要です。
- 5行目以下の部分が、物理キーの配列の定義です。
...
|名前|説明|
|-|-|
|QWERTY [JP]|日本語QWERTY配列|
|QWERTY [US]|英語QWERTY配列|
|Astarte [JP]|日本語QWERTY配列|
|Astarte [US]|英語QWERTY配列|
|Dvorak [US]|英語QWERTY配列|
|Eucalyn [JP]|日本語QWERTY配列|
|Eucalyn [US]|英語QWERTY配列|
|大西配列 [JP]|日本語QWERTY配列|
|大西配列 [US]|英語QWERTY配列|
|Tomisuke [JP]|日本語QWERTY配列|
...
英数字テーブルファイルは、拡張子を `chars.*.key` という名前にして、漢直WSのルートフォルダ配下の
`tables/_keyboard` に作成します。 

たとえば、大西配列であれば、次のようになります。

`tables/_keyboard/chars.onishi-jp.key`
...
シフトされたときに入力される文字配列は自動的に生成されますが、もしシフト文字も独自配列にしたい場合は、
`## SHIFT` ～ `## END` の間に文字を記述してください。(例: `chars.106.jp.txt` を参照)

また `\` のシフト文字が `|` になるか `_` になるかは、 `## YEN=nn` で指定しますが、指定のない場合は、
日本語キーボードなら最初に出現した `\` が、英語キーボードなら2つめに出現した `\` が対象となります。
...
また、「同時」の時間帯があってもそれを「同時打鍵」として扱うかどうかに関しては、さらに別の判定条件があります。
...
![Combo 1 2 50 80Ms](image/combo-1-2-50-80ms.png)
