...
- [IME連携](#IME連携)
    ...
    - [IME に送るローマ字定義を変更したい](#IME-に送るローマ字定義を変更したい)
- [同時打鍵](#同時打鍵)
...
数字は「[配列キーコード](../KEYBOARD.md#TOP)」と呼称しているものです。
...
### IME に送るローマ字定義を変更したい
IME をローマ字入力モードで使っている場合は、漢直WSでかなが入力されると、それに対応してデフォルトで設定してあるローマ字がIMEに対して送られます。
この「かな→ローマ字」変後定義を独自の設定にしたい場合は、漢直WSのルートフォルダにある `kana-roman.def.sample.txt` を `kana-roman.def.txt` という名前でコピーし、その内容を編集してください。

「っ」を入力したら `c` が送られるようにする修正例:
```
っ c
```

修正したファイルを保存した後、漢直WSの設定ダイアログなどで設定の「再読込」を実行してください。
...
![Both Charkeys Combo](image/both-charkeys-combo.png)
...
ただし、先押し後離し方式を採用している場合は、単打のロールオーバーのつもりで打鍵したものが、
意図せず2打鍵めが先に離されたと判断される可能性が高まるので、チェックを外しておいたほうが無難です。
...
![Katakana Cands](image/katakana-cands.png)
