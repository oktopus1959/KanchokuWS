...
対象となる被修飾キーの行の「割り当てキー／機能名」列のセルに割り当てるキー名や機能名、あるいは論理キーコードを入力します。
...
`A26` のような拡張シフト面に対する論理キーコードも記述することができます。
この場合もセル直接入力モードを使ってください。

### 論理キーコードの記法
基本面、通常シフト面、拡張シフト面A～F における論理キーコードを指定する場合は、次のように記述します。
#### 基本面 (0～99)
`0`～`99` の数字をそのまま記述します。

#### 通常シフト面 (100～199)
`100`～`199` の数字をそのまま記述します。あるいは、`S0`～`S99` と記述します。

#### 拡張シフト面A～F (200～799)
`200`～`799` の数字をそのまま記述します。
あるいは、各拡張シフト面ごとに、`A0`～`A99`、`B0`～`B99`、...、`F0`～`F99` と記述します。

#### 例
**`無変換`の単打に対して、拡張シフト面Fの`99`を割り当てる場合**

![Mod Conv Key Muhenkan F99](image/mod-conv-key-muhenkan-f99.png)
