...
例： 拡張シフトB面の「m」(論理キーコード:36)に交ぜ書き変換の呼び出しを設定する
```
-B36>@M
```
...
候補の文字数は、下図の「履歴文数字指定キーの論理キーコード」で指定したキーを押した後に
数字キーを押すことでも指定できます。
...
削除実行用のキーは、下図のように設定ダイアログの「履歴削除キーの論理キーコード」で変更することができます。
...
有効にしたい場合は、設定ダイアログの
「同時打鍵・IME」>「SandS」>「デコーダON時に SandS を有効にする」
