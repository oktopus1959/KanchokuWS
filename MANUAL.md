...
この機能を有効にするには、設定ダイアログの「その他設定」>「部首合成・連想直接置換」>
「自動部首合成を _N_ 回以上の実行で有効にする」をONにしてください。

![Auto Comp Dlg](image/auto-comp-dlg.png)

ただし、「 _N_ 回以上の実行で有効にする」の _N_ が2以上の場合は、指定回数以上、
手動の部首合成を実行した後に自動部首合成が有効になります。
...
「 _N_ 回以上の実行で有効にする」の _N_ を2以上に設定すると、その回数以上、
